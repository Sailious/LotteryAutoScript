...
const utils = {
    ...
    /**合集解析状态存放目录 */
    collections_dir: path.join(process.cwd(), 'collections'),
    /**
     * 将版本号转为数字
     * @example
     * 1.2.3 => 1.0203
    ...
    * infiniteNumber() {
     ...
     * @returns {fs.ReadStream}
     */
    readCollectionFile(num) {
        const fpath = num < 2 ? path.join(utils.collections_dir, 'collection.txt') : path.join(utils.collections_dir, `collection${num}.txt`);
        return fs.createReadStream(fpath, { encoding: 'utf8' });
    },
    /**
     * 追加合集解析状态
