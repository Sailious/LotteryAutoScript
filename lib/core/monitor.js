...
class Monitor extends Searcher {
            ...
            if (!hasOfficialLottery && model[1] === '1') {
                if (hasEnv('ENABLE_AI_JUDGE')) {
                    ...
                    try {
                        ...
                        if (msg && (msg.includes('是') || msg.toLowerCase().includes('true'))) {
                            has_key_words = true;
                            isLottery = true;
                            log.info('ai判断抽奖', '文本降级解析: 是');
                        } else if (msg && (msg.includes('否') || msg.toLowerCase().includes('false'))) {
                            has_key_words = false;
                            log.info('ai判断抽奖', '文本降级解析: 否');
                        } else {
    ...
    async go(option) {
        ...
        let
            ...
            { check_if_duplicated, is_copy_chat, copy_blockword, is_repost_then_chat, is_not_create_partition, wait } = config;
        ...
        await delay(wait * (Math.random() * 0.3 + 0.1));
