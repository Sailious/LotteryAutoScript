...
class Searcher {
     ...
     * @returns {boolean}
     */
    static isOpusCollectionDynamic(dynamicInfo) {
        if (!dynamicInfo || !dynamicInfo.description) {
            log.info('Opus合集检测', `动态(${dynamicInfo?.dynamic_id || 'unknown'})缺少description，判定为非合集`);
            return false;
        }

        const description = dynamicInfo.description;
        log.info('Opus合集检测', `动态(${dynamicInfo.dynamic_id})的description内容: ${description.substring(0, 200)}...`);

        /**提取t.bilibili.com链接 */
        const tBiliPattern = /t\.bilibili\.com\/(\d{18,})/g;
        const matches = [...description.matchAll(tBiliPattern)];

        log.info('Opus合集检测', `动态(${dynamicInfo.dynamic_id})找到${matches.length}个t.bilibili.com链接`);

        /**如果包含2个或以上的独立链接，认为是合集动态 */
        if (matches.length >= 2) {
            log.info('Opus合集检测', `动态(${dynamicInfo.dynamic_id})包含${matches.length}个独立链接，判定为合集动态`);
            return true;
        }

        log.info('Opus合集检测', `动态(${dynamicInfo.dynamic_id})包含${matches.length}个链接，判定为非合集`);
        return false;
    }
    ...
    async parseInnerLotteryLinksFromText(text, parent_dyid) {
        ...
        const tBiliPattern = /t\.bilibili\.com\/(\d{18,})/g;
        const matches = [...text.matchAll(tBiliPattern)];

        if (matches.length === 0) {
            return innerLotteryInfos;
        }

        log.info('深度解析', `文本中发现${matches.length}个t.bilibili.com链接`);

        for (const match of matches) {
            const innerDyid = match[1];
    ...
    async getLotteryInfoByUID(UID) {
        ...
        for (const cur of allModifyDynamicResArray) {
            ...
            if (cur.type === 1) {
                ...
                if (config.article_deep_parse && Searcher.isOpusCollectionDynamic(sourceDynamicInfo)) {
            ...
            /**类型3：其他非转发动态（纯文字、图文等）- 检查是否是Opus合集动态 */
                ...
                if (Searcher.isOpusCollectionDynamic(cur)) {
                    /**Opus合集动态：解析内嵌链接 */
    ...
    async getLotteryInfoByArticle(key_words) {
        ...
        log.info('获取动态', `article_deep_parse配置: ${config.article_deep_parse}`);
        ...
        for (const { id, pub_time } of cvs) {
            ...
            for (const dyid of dyid_set) {
                ...
                if (card) {
                    ...
                    /**立即检查是否是Opus合集动态 */
                    if (config.article_deep_parse && Searcher.isOpusCollectionDynamic(parsed_card)) {
