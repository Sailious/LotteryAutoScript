...
class Searcher {
     ...
     */
    async parseInnerLotteryLinksFromText(text, parent_dyid) {
        ...
        for (const innerDyid of allDyids) {
            ...
            try {
                const innerCard = await bili.getOneDynamicByDyid(innerDyid);
                if (innerCard) {
    ...
    async getLotteryInfoByUID(UID) {
        ...
        for (const cur of allModifyDynamicResArray) {
            index++;

            /**类型1：转发动态 - 获取源动态 */
            if (cur.type === 1) {
                const { origin_dynamic_id } = cur;
                let is_liked = false;

                if (!check_if_duplicated || check_if_duplicated >= 2) {
                    const card = await bili.getOneDynamicByDyid(origin_dynamic_id);
                    log.info('获取动态', `查看源动态(${origin_dynamic_id})是否点赞 (${index}/${length})`);
                    if (card) {
                ...
                /**检查源动态是否是Opus合集动态 */
                const sourceDynamicInfo = {
                    dynamic_id: cur.origin_dynamic_id,
                    description: cur.origin_description,
                    hasOfficialLottery: cur.origin_hasOfficialLottery,
                    type: cur.origin_type
                };

                let isOpusCollection = false;
                let opusLinks = [];
                if (config.article_deep_parse) {
                    const result = await Searcher.isOpusCollectionDynamic(sourceDynamicInfo);
                    isOpusCollection = result.isCollection;
                    opusLinks = result.links || [];
                }

                if (isOpusCollection) {
                    /**Opus合集动态：解析内嵌链接，不处理合集本身 */
                    log.info('获取动态', `源动态(${origin_dynamic_id})是Opus合集动态，解析内嵌链接`);
                    let innerLotteryInfos = [];
                    if (opusLinks.length > 0) {
                        /**如果已经有从HTML提取的链接，直接使用 */
                        innerLotteryInfos = await this.parseInnerLotteryLinksFromText(
                            opusLinks.map(link => `t.bilibili.com/${link}`).join(' '),
                            origin_dynamic_id
                        );
                    } else {
                        /**否则从description中提取 */
                        innerLotteryInfos = await this.parseInnerLotteryLinksFromText(
                            cur.origin_description,
                            origin_dynamic_id
                        );
                    }
                    if (innerLotteryInfos.length > 0) {
                        log.info('深度解析', `从Opus合集动态(${origin_dynamic_id})中解析出${innerLotteryInfos.length}个独立抽奖链接`);
            ...
            /**类型2：专栏动态（type === 64）- 获取专栏内容并提取链接 */
            else if (config.article_deep_parse && cur.type === 64) {
                log.info('获取动态', `处理专栏动态(${cur.dynamic_id}) (${index}/${length})`);
            ...
            /**类型3：其他非转发动态（纯文字、图文等）- 检查是否是Opus合集动态 */
            else if (config.article_deep_parse) {
                log.info('获取动态', `检查非转发动态(${cur.dynamic_id})是否是Opus合集 (${index}/${length})`);

                const result = await Searcher.isOpusCollectionDynamic(cur);
                if (result.isCollection) {
                    /**Opus合集动态：解析内嵌链接 */
                    log.info('获取动态', `动态(${cur.dynamic_id})是Opus合集动态，解析内嵌链接`);
                    let innerLotteryInfos = [];
                    ...
                    if (innerLotteryInfos.length > 0) {
                        log.info('深度解析', `从Opus合集动态(${cur.dynamic_id})中解析出${innerLotteryInfos.length}个独立抽奖链接`);
                        fomatdata.push(...innerLotteryInfos);
                    }
                }
                /**非合集动态且非转发动态，跳过（普通纯文字动态通常不是抽奖） */
            }
        }
        ...
        for (const { id, pub_time } of cvs) {
            ...
            for (const dyid of dyid_set) {
                ...
                if (card) {
                    ...
                    let opusLinks = [];
                    if (config.article_deep_parse) {
                        const result = await Searcher.isOpusCollectionDynamic(parsed_card);
                        isOpusCollection = result.isCollection;
                        opusLinks = result.links || [];
                    }

                    if (isOpusCollection) {
                        log.info('获取动态', `动态(${dyid})是Opus合集，解析内嵌链接`);
                        let innerLotteryInfos = [];
                        if (opusLinks.length > 0) {
                            /**如果已经有从HTML提取的链接，直接使用 */
                            innerLotteryInfos = await this.parseInnerLotteryLinksFromText(
                        ...
                        if (innerLotteryInfos.length > 0) {
                            log.info('深度解析', `从Opus合集动态(${dyid})中解析出${innerLotteryInfos.length}个独立抽奖链接`);
