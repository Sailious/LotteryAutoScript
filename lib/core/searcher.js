...
const fomatdata = await allModifyDynamicResArray
            ...
            .reduce(async (pre, cur) => {
                ...
                const sourceDynamicInfo = {
                    dynamic_id: cur.origin.dynamic_id,
                    description: cur.origin.description,
                ...
                if (isOpusCollection) {
                    ...
                    log.info('获取动态', `源动态(${cur.origin.dynamic_id})是Opus合集动态，解析内嵌链接`);
                    ...
                    if (opusLinks.length > 0) {
                        ...
                        innerLotteryInfos = await this.parseInnerLotteryLinksFromText(
                            ...
                            cur.origin.dynamic_id
                        );
                    } else {
                        /**否则从description中提取 */
                        innerLotteryInfos = await this.parseInnerLotteryLinksFromText(
                            cur.origin.description,
                            cur.origin.dynamic_id
                        );
                    }
                    if (innerLotteryInfos.length > 0) {
                        log.info('深度解析', `从Opus合集动态(${cur.origin.dynamic_id})中解析出${innerLotteryInfos.length}个独立抽奖链接`);
