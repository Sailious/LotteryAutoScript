...
function start(num) {
    return new Promise(resolve => {
        global_var.set('relayed_nums_sum', 0);
        ...
        event_bus.on('Turn_off_the_Monitor', async (msg) => {
            ...
            const relayed_sum = global_var.get('relayed_nums_sum') || 0;
            log.info('统计', `本次运行共参与${relayed_sum}个抽奖`);
            if (config.notice_running_state) {
                sendNotify(
                    `动态抽奖-帐号${num}结束抽奖-共参与${relayed_sum}个抽奖`,
                    `本次运行共参与${relayed_sum}个抽奖\n${log._cache.filter(it => /抽奖\]|Error|\s抽奖信息\]/.test(it)).join('\n')}`
                );
            }
